...
(function ($)
{
	...
	var caret = function (_iBegin, _iEnd)
		{
			...
			if (_iBegin)
			{
				...
				{
					this.setSelectionRange(_iBegin, _iEnd);
				}
				else if (this.createTextRange)
				{
					...
					oRange.moveEnd('character', _iEnd);
					oRange.moveStart('character', _iBegin);
					oRange.select();
				}
			}
			else
			{
				if (this.setSelectionRange)
				{
					_iBegin = this.selectionStart;
					_iEnd = this.selectionEnd;
				}
				else if (document.selection && document.selection.createRange)
				{
					var oRange = document.selection.createRange();

					_iBegin = 0 - oRange.duplicate().moveStart('character', -100000);
					_iEnd = _iBegin + oRange.text.length;
				}
	...
	$.fn.inputmasks = function(fn)
	{
		...
		{
			var nElem = this[0],
				oResult = maskMatch.call(nElem,
					(nElem.inputmask && nElem.inputmask._valueGet()) || nElem.value);

			return (oResult && oResult.completed);
		}
